...
class Solution {
  public List<Integer> replaceNonCoprimes(int[] nums) {
    ...
    for (int num : nums) {
      // using list as stack
      // Keep merging while the last number and current num
      // are non-coprime

      // first doing operations without adding num to the list

      while (!res.isEmpty()) {
        int last = res.get(res.size() - 1);
        int g = gcd(last, num);
        // int g = Math.gcd(last,num); // Java 14 and later

        if (g == 1) {
          break;
        }

        res.remove(res.size() - 1); // remove the last element
        long lcm = (long) last / g * num; // use long to avoid overflow
        num = (int) lcm; // changing num value
      }
  ...
  // Euclidean algorithm for GCD ( best )
